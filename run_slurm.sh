...
# ENABLE_PROFILING=0 sbatch -N ${NUM_GPU_NODE} -J seg-infer-${QLTY_WINDOW_SIZE}-mp-stitch --cpus-per-task ${CPUS_PER_GPU} submit_job.sh --yaml_path example_yamls/example_tunet.yaml --slurm_run_name ${CONFIG} --model_config ${MODEL_CONFIG}-${EXP_ID} --inference_batch_size ${INFERENCE_BATCH_SIZE} --qlty_window_size ${QLTY_WINDOW_SIZE} --qlty_step_size ${QLTY_STEP_SIZE} #-save_results